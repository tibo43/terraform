...
resource "aws_elastic_beanstalk_environment" "beanstalkAppName" {
  ...
  setting {
    ...
    value     = local.config.upperScaleIncrement
    resource  = ""
  }
  setting {
...
setting {
  ...
  setting {
    ...
    name      = "EvaluationTime"
    value     = local.config.evaluationTime
    resource  = ""
  }
  setting {
    namespace = "aws:elasticbeanstalk:trafficsplitting"
    name      = "NewVersionPercent"
    value     = local.config.newVersionPercent
    resource  = ""
  }
  setting {
...
resource "aws_lb_listener_rule" "forward_to_blog" {
  count = local.config.handle_blog ? 1 : 0
  listener_arn = data.aws_lb_listener.https_listener.arn
  priority = 1
  action {
    type = "forward"
    target_group_arn = "arn:aws:elasticloadbalancing:{aws_region}:{aws_account}:targetgroup/{aws_target_group_name}/{aws_target_group_id}"
  }
  condition {
    path_pattern {
      values = ["/blog*"]
...
resource "aws_lb_listener_rule" "redirect_to_blog" {
  count = local.config.handle_blog ? 1 : 0
  listener_arn = data.aws_lb_listener.https_listener.arn
  priority = 2
  action {
...
resource "aws_lb_listener_rule" "redirect_to_homepage" {
  count = local.config.handle_homepage ? 1 : 0
  listener_arn = data.aws_lb_listener.https_listener.arn
  priority = 3
  action {
    type = "redirect"
